...
struct LocationDetailView: View {
    ...
    var body: some View {
        ...
        }.ignoresSafeArea().background(.ultraThinMaterial).overlay(alignment: .topLeading) {
            backButton
        }
...
extension LocationDetailView {
    ...
    private var imageSection: some View {
        TabView {
            ForEach(location.imageNames, id: \.self) {
                Image($0).resizable().scaledToFill()
                    .frame(width: UIDevice.current.userInterfaceIdiom == .pad ? nil : UIScreen.main.bounds.width).clipped()
            }
    ...
    private var descriptionSection: some View {
        VStack(alignment: .leading, spacing: 16){
            ...
            if let url = URL(string: location.link) {
                Link("Read more on Wikipedia", destination: url).font(.headline).tint(.blue)
            }
    ...
    private var mapLayer: some View {
        ...
        }.allowsHitTesting(false).aspectRatio(1, contentMode: .fit).cornerRadius(30)
    }
